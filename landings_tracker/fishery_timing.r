...
dat <- subset(comp_dat_all,
  ...
  !SPECIES_GROUP == 'NON-WHITING GROUNDFISH NON-IFQ') %>%
  subset(AGENCY_CODE != 'F')
...
dat_summ <- mutate(dat_summconf,
  Species = #case_when(SPECIES_GROUP %in% c('Shoreside whiting', 'At-sea whiting') ~ SPECIES_GROUP, T ~ 
    convert_sp(SPECIES_GROUP) #)
  ,
...
dat_summ_rev <- dat %>%
  ...
  summarize(Revenue = median(Revenue)/1e6) %>%
