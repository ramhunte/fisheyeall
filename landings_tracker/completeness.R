...
setwd("R:/Confidential/FISHEyE/data/landings_tracker/")
...
addl <- alldat %>%
  subset(source %in% c(
    'comp_dat_raw2020-04-09.RDS',
    'comp_dat_raw2020-04-16.RDS',
    'comp_dat_raw2020-04-23.RDS',
    'comp_dat_raw2020-04-29.RDS',
    'comp_dat_raw2020-05-04.RDS',
    'comp_dat_raw2020-05-11.RDS',
    'comp_dat_raw2020-05-13.RDS',
    'comp_dat_raw2020-06-01.RDS')) %>%
  ungroup() %>%
...
ggsave(ggplot(subset(addl, LANDING_DATE > ymd('2020-03-14')), aes(x = LANDING_DATE, y = cumMTS)) + 
  geom_line(aes(color = source)) + 
  facet_grid(AGENCY_CODE ~ ., scales = 'free_y'), file = 'completeness.png')
...
ggplot(addlwa, aes(x = LANDING_DATE, y = cumMTS)) + 
  geom_line(aes(color = source)) + 
